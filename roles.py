...
async def roles(message):
    ...
    if len(message_splitted) <= 1 or message_splitted[1] == "help":
        ...
        return
    if message_splitted[1] == "help":
        await help.help_access(message)
        return
    elif message_splitted[1] == "add":
        await add_reactionrole(message, message_splitted)
        return
    elif message_splitted[1] == "remove":
        await remove_reactionrole(message, message_splitted)
        return
    elif message_splitted[1] == "set":
        await set_channel_message(message, message_splitted)
        return
    elif message_splitted[1] == "messages":
...
async def add_reactionrole(message, message_splitted):
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS '"+ message_id +"' (emoji text, role_id integer, channel_id integer)")
    ...
    cursor.execute("INSERT INTO '"+message_id+"' VALUES ('"+used_emoji+"',"+str(role_id)+","+str(active_message.id)+")")
    cursor.execute("""INSERT OR REPLACE INTO reaction_role_messages (
        message_id,
        channel_id)VALUES(
            """+message_id+""",
...
async def remove_reactionrole(message, message_splitted):
    ...
    message_id = str(active_message.id)
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='"+str(message_id)+"'")
    ...
    cursor.execute("DELETE FROM '"+message_id+"' WHERE emoji='"+used_emoji+"'")
    cursor.execute("SELECT * FROM '"+message_id+"'")
    #If last reaction from the message is removed, drop the table for that message.
    messages = cursor.fetchall()
    if len(messages) < 1:
        cursor.execute("DROP TABLE '"+message_id+"'")
...
async def set_channel_message(message, message_splitted):
    ...
    cursor.execute("""INSERT OR REPLACE INTO active_messages (name, active_channel_id,active_message_id)
                            VALUES(
                                'roles',
                                """+str(ch[0].id)+""",
